 ...
 * - Opens scheduled            // Queue opening notification email
            const { error: openingEmailError } = await supabase
                ...
                .insert([{
                    ...
                    email_type: 'order_opened'
                }]);en their opening time arrives
        ...
        try {
            ...
            const { error: closureEmailError } = await supabase
                ...
                .insert([{
                    ...
                    status: 'failed', // 'failed' status is used as queue
                    email_type: 'order_closed'
                }]);

            if (closureEmailError) {
                log('error', 'Error queueing closure email', { 
            ...
            const { error: summaryEmailError } = await supabase
                ...
                .insert([{
                    ...
                    status: 'failed', // 'failed' status is used as queue
                    email_type: 'order_summary'
                }]);

            if (summaryEmailError) {
                log('error', 'Error queueing summary email', { 
